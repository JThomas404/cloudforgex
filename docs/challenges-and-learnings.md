...
async function requestCertificateUrl(certificateName) {
  try {
    const response = await fetch('https://api.jarredthomas.cloud/certificates', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json'
      },
    ...
    if (data.url) {
      ...
      modal.innerHTML = `
        <div class="modal-content">
          ...
          <div class="expiry-notice">Link expires in ${Math.floor(data.expires_in / 60)} minutes</div>
        </div>
      `;
...
This document demonstrates systematic problem-solving approaches, technical depth, and production-level thinking applied to real challenges in the CloudForgeX project. The solutions implemented showcase engineering maturity, thoughtful decision-making, and a focus on both technical excellence and business value.